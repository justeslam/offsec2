...
Write-Host "Words: ${words}       Ext's: ${fileExtensions}       Pattern: ${pattern}     Regex Search: ${regexSearch}"
...
foreach ($searchPath in $searchPaths) {
    if (Test-Path $searchPath) {
        try {
            $foundFiles = Get-ChildItem -Path "${searchPath}" -Recurse -Include "${fileExtensions}" -ErrorAction SilentlyContinue -Force |
                Where-Object {
